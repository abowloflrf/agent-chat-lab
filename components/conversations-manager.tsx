...
import { formatFullDateTime, formatMessageDateTime } from "@/lib/datetime";
...
export function ConversationsManager() {
  ...
  const deleteOne = useCallback(
    (row: ConversationStat) => {
      const label = row.title ?? DEFAULT_CONVERSATION_TITLE;
      if (window.confirm(`确定删除会话「${label}」？此操作不可撤销。`)) {
  ...
  const deleteSelected = useCallback(() => {
    ...
    if (window.confirm(`确定删除选中的 ${ids.length} 个会话？此操作不可撤销。`)) {
      void deleteIds(ids);
    }
        ...
        {selectedVisibleCount > 0 ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={busy}
              className="rounded-full bg-[#9a3412] px-3 py-1.5 text-xs font-medium text-white transition hover:bg-[#7c2d12] disabled:opacity-50"
            >
              删除所选
            </button>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="max-w-[260px] px-2 py-2.5">
                      <span className="block truncate text-[#352d25]" title={title}>
                        {title}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-2 py-2.5 font-mono text-[#6d6257]" title={formatFullDateTime(createdMs)}>
                      {formatMessageDateTime(createdMs)}
                    </td>
                    <td className="whitespace-nowrap px-2 py-2.5 text-[#6d6257]" title={formatFullDateTime(lastMs)}>
                    ...
                    <td className="px-2 py-2.5 text-right font-mono tabular-nums text-[#9c5626]">
                      {row.cacheHitRate === null ? DASH : `${formatCacheHitRate(row.cacheHitRate)}%`}
                    </td>
                    <td className="px-2 py-2.5 text-right font-mono tabular-nums text-[#53483d]">
                      {row.userTurns}
                    </td>
                    <td className="max-w-[160px] px-2 py-2.5">
                      <span className="block truncate font-mono text-[12px] text-[#6d6257]" title={row.lastModelId ?? undefined}>
                        {row.lastModelId ?? DASH}
                      </span>
                    </td>
                    <td className="px-2 py-2.5 text-right font-mono tabular-nums text-[#53483d]">
