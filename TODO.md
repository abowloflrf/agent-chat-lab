...
- [x] 优化左上角模块切换器的点击外部自动收起交互
- [x] 调整左上角模块切换器的紧凑度与下拉 hover 间距
- [x] 调整左上角模块切换器的模块顺序与英文名称
- [x] 修复未登录时 `manifest.webmanifest` 被认证代理重写为 HTML 的问题
- [x] 重做 TODO 页 UI，包含右侧默认留白、标题层级强化、Markdown 等宽正文、分段式状态选择，以及左侧仅标题清单和 `! / !!` 优先级标记
- [x] 修复全局表单字体重置覆盖组件字号和字重的问题
