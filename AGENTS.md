...
Keep `TODO.md` and `CHANGELOG.md` in sync with the codebase, but do not update them too frequently for minor routine changes. `TODO.md` should contain only meaningful unfinished work, while completed work and significant code improvements should be recorded in `CHANGELOG.md`. Keep `CHANGELOG.md` entries concise and user-facing; do not include implementation details. Simple bugfixes, small UI fixes, ordinary small fixes, and low-impact refactors do not need to be recorded. Only update these files for key code changes, feature updates, important fixes, scope changes, or priority changes. When such tracked changes happen, update the relevant file in the same change so the repository always reflects current project status. If repository guidance changes materially, update `AGENTS.md` in the same pass.
