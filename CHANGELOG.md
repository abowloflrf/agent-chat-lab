...
## 2026-07-29
- 工具结果（含 MCP）统一施加输出上限（可用 `TOOL_OUTPUT_MAX_BYTES` 调整），超限时完整输出落盘、模型可按路径回查，WebFetch 不再重复返回正文预览：长会话上下文体积实测下降约 19%

## 2026-07-07
- 输入框的模型选择下拉支持输入关键字快速搜索模型

## 2026-07-03
- 后端补齐结构化日志（pino）：聊天请求生命周期、Bash 执行、Web 搜索故障转移、MCP 连接与 OAuth、数据库初始化、登录与配置变更等关键操作现可在服务端日志追踪

## 2026-06-26
- 设置页可自定义正文 / 等宽字体（可填多个、逗号分隔，实时预览），保存在服务端、跨设备生效、无首屏闪烁
- 用户消息中超长的粘贴内容（大段日志/文档）自动折叠为卡片，显示行数与字数，点击展开为等宽可滚动纯文本
- Bash 工具调用改为终端式展示：命令与 stdout/stderr 合并为终端块，显示退出码与耗时，可一键展开看完整命令与输出

## 2026-06-24
- 输入框支持语音输入

## 2026-06-22
- MCP Server 支持 OAuth 认证：远程 MCP Server 可选 Header 或 OAuth，授权一次后聊天自动使用并续期
- MCP 健壮性改进：连接跨请求复用、工具调用超时、工具名按服务器前缀隔离

## 2026-06-21
- 支持多套配色主题切换（暖棕奶油/素雅黑白/OLED 深色），设置页可选，仅存浏览器、无首屏闪烁
- 联网回复新增来源列表与正文行内引用胶囊（显示来源域名，点击查看来源卡片）
...
## 2026-06-14
- 接入 Exa 联网搜索工具
- 重构 chat-shell：渲染层拆为 6 个展示子组件、纯函数入 lib/chat-utils、token 统计合并 memo（行为不变）
